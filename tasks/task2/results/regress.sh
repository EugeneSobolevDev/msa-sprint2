...
# 8. Event создан в Kafka, обработан booking-history-service и сохранен в booking_history
echo ""
echo "Тесты booking-history"
echo "Проверка подключения к ${HIST_DB_HOST}:${HIST_DB_PORT}..."
timeout 5 bash -c "echo >/dev/tcp/${HIST_DB_HOST}/${HIST_DB_PORT}" && \
  echo "Соединение есть" || echo "Нет соединения"

PGPASSWORD="${HIST_DB_PASSWORD}" psql -h "${HIST_DB_HOST}" -p "${HIST_DB_PORT}" -U "${HIST_DB_USER}" -d "${HIST_DB_NAME}" \
  -c "SELECT user_id FROM booking_history;" > /tmp/booking_history.out 2>&1

grep -q "test-user-2" /tmp/booking_history.out && grep -q "test-user-3" /tmp/booking_history.out \
  && pass "История бронирований найдена в booking_history" \
  || fail "История бронирований не найдена в booking_history или ошибка подключения к БД"
